...
class Attention(nn.Module):
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        layer_id = cross_attention_kwargs['layer_id'] if 'layer_id' in cross_attention_kwargs.keys() else -1
        cross_attention_kwargs = {k: w for k, w in cross_attention_kwargs.items() if k in attn_parameters}
        cross_attention_kwargs['layer_id'] = layer_id
...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs
        ...
        attn = (query[0].float().cpu() @ key[0].float().cpu().transpose(-1,-2) / query.shape[-1]**0.5).softmax(-1)
        size = 256
        attn_scaled = torchvision.transforms.Resize((size, size), interpolation=torchvision.transforms.InterpolationMode.NEAREST)(attn)
        torch.save(attn_scaled, f"./metadata/layer{layer_id}_attn_{size}_cond.pth")
